...
//  LeetCodeProblems
//
//  Created by Aynur Nasybullin on 04.03.2024.
//
...
extension Problems {
    ...
    // Time complexity: O(n log n) =  ms
    // Space complexity: O(n) = 16.43 MB
    static func problem_948_bagOfTokensScore(_ tokens: [Int], _ power: Int) -> Int {
        var points = 0
        if tokens.isEmpty { return points }
        if tokens.count == 1 {
        ...
        if power >= sortedTokens[left] { points += 1 }
