...
//  LeetCodeProblems
//
//  Created by Aynur Nasybullin on 19.03.2024.
//
...
//    In the 3rd interval, neither A nor B can be done, so you idle. By the 4th cycle,
...
extension Problems {
    ...
    // Approach: Math, Hash Table, Counting
    // Time complexity: O(n) => 238 ms
    // Space complexity: O(n) => 16.55 MB
    public func problem_621_leastInterval(_ tasks: [Character], _ n: Int) -> Int {
        ...
        return max(tasks.count, (maxCount - 1) * (n + 1) + nMaxCount)
    }
}


extension ProblemsTestCases {
    static func testsData_621() -> [(tasks: [Character], n: Int, expected: Int)] {
        var testsData = [(tasks: [Character], n: Int, expected: Int)]()
