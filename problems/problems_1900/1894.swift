...
class Solution {
    func chalkReplacer(_ chalk: [Int], _ k: Int) -> Int {
        ...
        var prefixSum = Array(repeating: chalk[0], count: n)
        for idx in 1..<n {
            prefixSum[idx] = prefixSum[idx - 1] + chalk[idx]
        }

        let remains = k - ((k / prefixSum[n - 1]) * prefixSum[n - 1])
        
        var (left, right) = (0, n - 1)
        while left < right {
            let middle = (left + right + 1) / 2

            if prefixSum[middle] > remains {
                if prefixSum[middle - 1] > remains {
                    right = middle - 1
                } else { 
                    return middle
                }
