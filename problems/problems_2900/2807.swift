...
class Solution {
    func insertGreatestCommonDivisors(_ head: ListNode?) -> ListNode? {
        guard var current = head, var next = current.next else { return head }

        while true {
            var val = max(current.val, next.val)
            var divider = min(current.val, next.val)
            var remainder = val % divider

            while remainder > 1 {
                val = divider
                divider = remainder
                remainder = val % divider
            }
            ...
            current.next = ListNode(divider, next)

            if let next2 = next.next {
                current = next
                next = next2
            } else {
                return head
